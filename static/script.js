...
var format = function (curPost) {
    ...
    var post = "<div class='container posthold' style='margin-bottom: 10px;'><h1 class='display-1'>" + curPost.title + "</h1>";
    var formatPost = curPost.content.replace(/\n/g, "<br/>");
    //replaces line breaks with br to ensure that it will properly display on the page
    post += "<p class='lead'> By: " + curPost.username + " on " + curPost.date + "</p><hr >";
...
var displayTitles = function () {
    ...
    $.get("/allposts/", function (data) {
        ...
        $("#postContainer").empty();
        //put in a table
        $("#postContainer").append("<table class ='table table-inverse'></table>");
        $("#postContainer").find("table").append("<thead><tr><td>Post Title</td> <td>Author</td> <td>Date</td> </tr> </thead>");
        //put in a head
        for (var i = 0; i < d.length; i++) {
            $("#postContainer").find("table").append("<tr> <td>" + d[i].title + "</td><td>" + d[i].username + "</td> <td>" +
                d[i].date + "</td><td><button class='btn btn-block btn-secondary btnShowMore' id = " + d[i].id + ">View Post</button></td>+ </tr>");
...
//adding these buttons for every post it makes sense to use .on instead of just click
...
$("#postContainer").on("click", ".delete", function (e) {
    ...
    //sends an ajax delete request
    $.ajax({
        url: ('/userposts/' + myID),
        type: 'DELETE',
        success: function (data) {
            //on success go back to their personal view
